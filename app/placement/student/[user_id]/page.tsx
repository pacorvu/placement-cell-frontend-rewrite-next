"use client";

import { useState, use } from "react";
import ProjectsForm from "@/components/placement/student/project";
import CertificationsForm from "@/components/placement/student/certifications";
import PublicationsForm from "@/components/placement/student/publications";
import OtherExperiencesForm from "@/components/placement/student/other";
import SemesterAcademicsForm from "@/components/placement/student/academics";
import { toast } from "sonner";
import ProfileDetailsForm from "@/components/placement/student/details";
import ProfileCommunicationForm from "@/components/placement/student/contact";
import ParentDetailsForm from "@/components/placement/student/parents";
import PersonalDetailsForm from "@/components/placement/student/personal-information";
import EducationalHistoryForm from "@/components/placement/student/educational-history";
import InternshipForm from "@/components/placement/student/internship";
import TrainingsForm from "@/components/placement/student/trainings";
import ExtracurricularForm from "@/components/placement/student/extra-ciricular";

const sidebarItems = [
  {
    id: "personal",
    label: "Personal Information",
    hasData: true,
    icon: <i className="material-icons h-5 w-5">person</i>
  },
  {
    id: "contact",
    label: "Contact & Links",
    hasData: false,
    icon: <i className="material-icons h-5 w-5">email</i>
  },
  {
    id: "parent",
    label: "Parent / Guardian Details",
    hasData: false,
    icon: <i className="material-icons h-5 w-5">family_restroom</i>
  },
  {
    id: "career",
    label: "Career Overview",
    hasData: false,
    icon: <i className="material-icons h-5 w-5">work</i>
  },
  {
    id: "education",
    label: "Education",
    hasData: false,
    icon: <i className="material-icons h-5 w-5">school</i>
  },
  {
    id: "academic",
    label: "Academic Performance",
    hasData: false,
    icon: <i className="material-icons h-5 w-5">bar_chart</i>
  },
  {
    id: "projects",
    label: "Projects",
    hasData: false,
    addable: true,
    icon: <i className="material-icons h-5 w-5">code</i>
  },
  {
    id: "internships",
    label: "Internships",
    hasData: false,
    addable: true,
    icon: <i className="material-icons h-5 w-5">business_center</i>
  },
  {
    id: "training",
    label: "Training & Workshops",
    hasData: false,
    addable: true,
    icon: <i className="material-icons h-5 w-5">local_library</i>
  },
  {
    id: "certifications",
    label: "Certifications",
    hasData: false,
    addable: true,
    icon: <i className="material-icons h-5 w-5">verified</i>
  },
  {
    id: "publications",
    label: "Publications",
    hasData: false,
    addable: true,
    icon: <i className="material-icons h-5 w-5">article</i>
  },
  {
    id: "extracurricular",
    label: "Extra-Curricular Activities",
    hasData: false,
    addable: true,
    icon: <i className="material-icons h-5 w-5">star</i>
  },
  {
    id: "experiences",
    label: "Other Experiences",
    hasData: false,
    addable: true,
    icon: <i className="material-icons h-5 w-5">work_outline</i>
  },
];

export default function StudentProfile({ params }:{ params: Promise<{ user_id: number }> }) {
  const userId: number = (use(params)).user_id
  const [activeSection, setActiveSection] = useState("personal");
  const [isEditing, setIsEditing] = useState(false);
  return (
    <div className="min-h-screen bg-base-100">
      {/* Main Content */}
      <div className="flex max-w-7xl mx-auto flex-col lg:flex-row">
        {/* Sidebar */}
        <aside className="w-full lg:w-64 bg-base-200 border-r border-base-300">
          <div className="p-4">
            <h2 className="text-sm font-bold text-base-content/60 uppercase mb-4">
              Profile Sections
            </h2>
            <nav className="space-y-1">
              {sidebarItems.map((item) => (
                <button
                  key={item.id}
                  onClick={() => setActiveSection(item.id)}
                  className={`w-full text-left px-3 py-2 rounded-lg transition-all text-sm flex items-center gap-3 ${activeSection === item.id
                    ? "bg-primary text-primary-content font-medium"
                    : "hover:bg-base-300 text-base-content"
                    }`}
                >
                  {item.icon}
                  <span className="flex-1">{item.label}</span>
                  {/* {item.hasData && activeSection !== item.id && (
                    <span className="badge badge-success badge-xs"></span>
                  )} */}
                </button>
              ))}
            </nav>
          </div>
        </aside>

        {/* Main Content Area */}
        <main className="flex-1 p-8">
          {activeSection === "personal" && (
            <PersonalDetailsForm
              userId={userId}
              onSuccess={() => toast.success("Updated successfully")}
              onError={(err) => toast.error(err.message)}
            />
          )}

          {activeSection === "contact" && (
            <ProfileCommunicationForm
              userId={userId}
              onSuccess={() => toast.success("Updated successfully")}
              onError={(err) => toast.error(err.message)}
            />
          )}

          {activeSection === "parent" && (
            <ParentDetailsForm
              userId={userId}
              onSuccess={() => toast.success("Updated successfully")}
              onError={(err) => toast.error(err.message)}
            />
          )}

          {activeSection === "career" && (
            <ProfileDetailsForm
              userId={userId}
              onSuccess={() => toast.success("Updated successfully")}
              onError={(err) => toast.error(err.message)}
            />

          )}

          {activeSection === "education" && (
            <EducationalHistoryForm
              userId={userId}
              onSuccess={() => toast.success("Updated successfully")}
              onError={(err) => toast.error(err.message)}
            />
          )}

          {activeSection === "academic" && (
            <SemesterAcademicsForm
              userId={userId}
              onSuccess={() => toast.success("Updated successfully")}
              onError={(err) => toast.error(err.message)}
            />
          )}

          {activeSection === "projects" && (
            <ProjectsForm
              userId={userId}
              onSuccess={() => toast.success("Updated successfully")}
              onError={(err) => toast.error(err.message)}
            />
          )}

          {activeSection === "internships" && (
            <InternshipForm
              userId={userId}
              onSuccess={() => toast.success("Updated successfully")}
              onError={(err) => toast.error(err.message)}
            />
          )}

          {activeSection === "training" && (
            <TrainingsForm
              userId={userId}
              onSuccess={() => toast.success("Updated successfully")}
              onError={(err) => toast.error(err.message)}
            />
          )}

          {activeSection === "certifications" && (
            <CertificationsForm
              userId={userId}
              onSuccess={() => toast.success("Updated successfully")}
              onError={(err) => toast.error(err.message)}
            />
          )}

          {activeSection === "publications" && (
            <PublicationsForm
              userId={userId}
              onSuccess={() => toast.success("Updated successfully")}
              onError={(err) => toast.error(err.message)}
            />
          )}

          {activeSection === "extracurricular" && (
            <ExtracurricularForm
              userId={userId}
              onSuccess={() => toast.success("Updated successfully")}
              onError={(err) => toast.error(err.message)}
            />
          )}

          {activeSection === "experiences" && (
            <OtherExperiencesForm
              userId={userId}
              onSuccess={() => toast.success("Updated successfully")}
              onError={(err) => toast.error(err.message)}
            />
          )}

          {/* {activeSection === "resume" && ( */}
          {/*   <ResumeForm isEditing={isEditing} /> */}
          {/* )} */}
        </main>
      </div>
    </div>
  );
}
