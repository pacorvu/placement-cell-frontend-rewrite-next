...
interface Activity { id:number; name:string; role:string; achievement:string; date:string; proof?:File|null }
interface ExtraCurricularFormProps { isEditing:boolean }

export default function ExtraCurricularForm({ isEditing }: ExtraCurricularFormProps){
  const [items,setItems]=useState<Activity[]>([]);
  const add=()=>setItems(x=>[...x,{id:Date.now(),name:'',role:'',achievement:'',date:'',proof:null}]);
  const remove=(id:number)=>setItems(x=>x.filter(i=>i.id!==id));
  const update=(id:number,data:Partial<Activity>)=>setItems(x=>x.map(i=>i.id===id?{...i,...data}:i));
    ...
    <div className="space-y-6">
      <div className="card bg-base-100 shadow border border-base-300">
        <div className="card-body">
          ...
          {items.map((it,idx)=> (
            <div key={it.id} className="mt-4 p-4 border rounded bg-base-50 relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                <div>
                  <label className="block text-sm text-muted">Activity Name</label>
                  <input value={it.name} onChange={(e)=>update(it.id,{name:e.target.value})} disabled={!isEditing} className="w-full px-3 py-2 border rounded mt-2" />
                </div>
                ...
                <div>
                  <label className="block text-sm text-muted">Achievement / Description</label>
                  <input value={it.achievement} onChange={(e)=>update(it.id,{achievement:e.target.value})} disabled={!isEditing} className="w-full px-3 py-2 border rounded mt-2" />
                </div>
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm text-muted">Upload Certificate/Proof</label>
          ...
          <div className="mt-4">
            <button type="button" className="btn btn-ghost btn-block" onClick={add}><i className="material-icons mr-2">add</i> Add Activity</button>
          </div>
        </div>
      </div>
    </div>
