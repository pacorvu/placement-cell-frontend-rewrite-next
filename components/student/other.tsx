...
const createOtherExperiencesRequestSchema = z.object({
...
const updateOtherExperiencesRequestSchema = z.object({
	description: z.string().nullable(),
	end_date: z.string().nullable(),
	location: z.string().nullable(),
	organization: z.string().nullable(),
...
>;
type CreateOtherExperiencesRequest = z.infer<
	typeof createOtherExperiencesRequestSchema
>;
type UpdateOtherExperiencesRequest = z.infer<
	typeof updateOtherExperiencesRequestSchema
>;
...
function AddOtherExperienceForm({
	...
	const createMutation = useMutation({
		mutationFn: async (values: CreateFormValues) => {
			console.log("=== CREATING NEW OTHER EXPERIENCE ===");
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						<FormField label="Organization" htmlFor="organization_new">
							<input
								...
								placeholder="e.g., University Lab, Competition Name"
								className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100"
							/>
						</FormField>
		...
		<div className="border border-gray-200 rounded-lg overflow-hidden">
			...
			{isExpanded && (
				<form onSubmit={handleSubmit} className="p-6 space-y-4 bg-white">
					...
					{/* Title */}
					<FormField label="Experience Title" htmlFor={`title_${record.id}`}>
						<input
							id={`title_${record.id}`}
							type="text"
							value={formValues.title ?? ""}
